...
// Feistel cipher (advanced version for N trails and possible
...
class trail_adv
{
  ...
  // Overall Information
  std::vector<std::vector<round_info>> fin_trails;
  std::vector<std::vector<float>> fin_biases;
  ...
  // Constructor
  trail_adv(const feistel& cipher);
  ...
  // Trail Masks
  std::vector<std::tuple<bitstr, bitstr, bitstr>> trail_masks(size_type rounds);
  
  // Helpers
  void place(std::vector<round_info> &arr, round_info entry);
  bool is_equal(const round_info& a, const round_info& b);
};
